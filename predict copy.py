...
dp[0][0] = PB * (predictions[0])
dp[1][0] = PN * (1-predictions[0])

for i in range(1,len(predictions)):
    #boundary
    dp[0][i] = predictions[i] * max(dp[0][i-1] * PBB, dp[1][i-1]*PNB)
    #nb
    dp[1][i] = predictions[i] * max(dp[0][i-1] * PBN, dp[1][i-1]*PNN)
...
sump = dp[0][len(predictions)-1] + dp[1][len(predictions)-1]
randomp = pow(0.5, len(predictions))
print sump
print randomp
sys.exit()
'''
